...
public class BigTableScanner implements ISSTableScanner
{
    ...
    {
        return new BigTableScanner(sstable, null, limiter, false, Iterators.singletonIterator(fullRange(sstable)));
    }

    public static ISSTableScanner getScanner(SSTableReader sstable, DataRange dataRange, RateLimiter limiter, boolean isForThrift)
    {
        return new BigTableScanner(sstable, dataRange, limiter, isForThrift, makeBounds(sstable, dataRange).iterator());
    }
    ...
    public static ISSTableScanner getScanner(SSTableReader sstable, Collection<Range<Token>> tokenRanges, RateLimiter limiter)
    {
        ...
        return new BigTableScanner(sstable, null, limiter, false, makeBounds(sstable, tokenRanges).iterator());
    }

    private BigTableScanner(SSTableReader sstable, DataRange dataRange, RateLimiter limiter, boolean isForThrift, Iterator<AbstractBounds<PartitionPosition>> rangeIterator)
    {
        assert sstable != null;
                ...
                return new LazilyInitializedUnfilteredRowIterator(currentKey)
                {
                    protected UnfilteredRowIterator initializeIterator()
                    {
                        try
                        {
                            ...
                            return filter.filter(sstable.iterator(dfile, partitionKey(), currentEntry, filter.queriedColumns(), filter.isReversed(), isForThrift));
                        }
