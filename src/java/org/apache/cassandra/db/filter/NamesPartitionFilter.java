...
public class NamesPartitionFilter extends AbstractPartitionFilter
{
    ...
    public NamesPartitionFilter(ColumnsSelection columns, NavigableSet<Clustering> clusterings, boolean reversed)
    {
        ...
        assert !clusterings.isEmpty() || !columns.fetchedColumns().statics.isEmpty();
    ...
    {
        this(ColumnsSelection.selection(columns), clusterings, reversed);
    }
    ...
    public UnfilteredRowIterator filterNotIndexed(UnfilteredRowIterator iterator)
    {
    ...
    public UnfilteredRowIterator getUnfilteredRowIterator(final Partition partition)
    {
        final SearchIterator<Clustering, Row> searcher = partition.searchIterator(queriedColumns, reversed);
        return new AbstractUnfilteredRowIterator(partition.metadata(),
                                        partition.partitionKey(),
                                        partition.partitionLevelDeletion(),
                                        queriedColumns.fetchedColumns(),
    ...
    public int maxQueried(boolean countCells)
    {
        ...
        if (clusterings.isEmpty())
            return countCells ? queriedColumns.fetchedColumns().statics.columnCount() : 1;

        return countCells ? clusterings.size() * queriedColumns.fetchedColumns().regulars.columnCount() : clusterings.size();
    }
