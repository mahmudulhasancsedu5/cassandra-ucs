    ...
    protected UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs, boolean copyOnHeap)
    {
        ...
        try
        {
            for (Memtable memtable : view.memtables)
            {
                ...
                UnfilteredRowIterator iter = filter.getUnfilteredRowIterator(partition);
            ...
            for (SSTableReader sstable : view.sstables)
            {
                ...
                @SuppressWarnings("resource") // 'iter' is added to iterators which is closed on exception, or through the closing of the final merged iterator
                UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), filter.queriedColumns(), filter.isReversed(), isForThrift()));
