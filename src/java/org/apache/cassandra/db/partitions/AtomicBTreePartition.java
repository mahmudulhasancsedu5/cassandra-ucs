...
public class AtomicBTreePartition implements Partition
{
    ...
    public SearchIterator<Clustering, Row> searchIterator(final ColumnsSelection columns, final boolean reversed)
    {
    ...
    public UnfilteredRowIterator unfilteredIterator(ColumnsSelection selection, Slices slices, boolean reversed)
    {
        if (slices.size() == 0)
        {
            Holder current = ref;
    ...
    private static Row makeStatic(ColumnsSelection selection, Holder holder, MemtableAllocator allocator)
    {
