...
public abstract class AbstractPartitionData implements Partition, Iterable<Row>
{
    ...
    {
        Row row = searchIterator(ColumnsSelection.selection(columns()), false).next(clustering);
    ...
    public SearchIterator<Clustering, Row> searchIterator(final ColumnsSelection columns, boolean reversed)
    {
    ...
    {
        return unfilteredIterator(ColumnsSelection.selection(columns()), Slices.ALL, false);
    }

    public UnfilteredRowIterator unfilteredIterator(ColumnsSelection columns, Slices slices, boolean reversed)
    {
    ...
    {
        return sliceableUnfilteredIterator(ColumnsSelection.selection(columns()), false);
    }

    protected SliceableUnfilteredRowIterator sliceableUnfilteredIterator(final ColumnsSelection selection, final boolean reversed)
    {
    ...
    private RowIterator createRowIterator(ColumnsSelection columns, boolean reversed)
    {
    ...
    private abstract class RowIterator extends UnmodifiableIterator<Row>
    {
        ...
        protected RowIterator(final ColumnsSelection columns)
        {
    ...
    {
        private ForwardRowIterator(ColumnsSelection columns)
        {
            super(columns);
            this.next = 0;
        }
    ...
    {
        private ReverseRowIterator(ColumnsSelection columns)
        {
            super(columns);
            this.next = rows - 1;
        }
