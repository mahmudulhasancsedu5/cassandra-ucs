...
public class SinglePartitionNamesCommand extends SinglePartitionReadCommand<NamesPartitionFilter>
{
    ...
    protected UnfilteredRowIterator queryMemtableAndDiskInternal(ColumnFamilyStore cfs, boolean copyOnHeap)
    {
        ...
        for (Memtable memtable : view.memtables)
        {
            ...
            try (UnfilteredRowIterator iter = filter.getUnfilteredRowIterator(partition))
            {
                if (iter.isEmpty())
                    continue;
        ...
        for (SSTableReader sstable : view.sstables)
        {
            ...
            try (UnfilteredRowIterator iter = filter.filter(sstable.iterator(partitionKey(), filter.queriedColumns(), filter.isReversed(), isForThrift())))
            {
                if (iter.isEmpty())
                    continue;
            ...
            && cfs.getCompactionStrategyManager().shouldDefragment())
        {
            ...
            try (UnfilteredRowIterator iter = result.unfilteredIterator(queriedColumns(), Slices.ALL, false))
            {
        ...
        return result.unfilteredIterator(queriedColumns(), Slices.ALL, partitionFilter().isReversed());
    }
    ...
    private ArrayBackedPartition add(UnfilteredRowIterator iter, ArrayBackedPartition result)
    {
        ...
        try (UnfilteredRowIterator merged = UnfilteredRowIterators.merge(Arrays.asList(iter, result.unfilteredIterator(queriedColumns(), Slices.ALL, false)), nowInSec()))
        {
    ...
    private NamesPartitionFilter reduceFilter(NamesPartitionFilter filter, Partition result, long sstableTimestamp)
    {
        ...
        SearchIterator<Clustering, Row> searchIter = result.searchIterator(queriedColumns(), false);

        PartitionColumns columns = filter.queriedColumns().fetchedColumns();
