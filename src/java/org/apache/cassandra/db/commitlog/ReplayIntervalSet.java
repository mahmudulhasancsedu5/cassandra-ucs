...
public class ReplayIntervalSet
{
    ...
    public static final ISerializer<ReplayIntervalSet> serializer = new ISerializer<ReplayIntervalSet>()
    {
        ...
        public ReplayIntervalSet deserialize(DataInputPlus in) throws IOException
        {
        ...
        public long serializedSize(ReplayIntervalSet intervals)
        {
            long size = TypeSizes.sizeof(intervals.ranges.size());
            for (Map.Entry<ReplayPosition, ReplayPosition> en : intervals.ranges.entrySet())
            {
                size += ReplayPosition.serializer.serializedSize(en.getKey());
                size += ReplayPosition.serializer.serializedSize(en.getValue());
            }
            return size;
        }
    };
