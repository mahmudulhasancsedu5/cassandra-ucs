 ...
 */
public abstract class ConcreteCollectionType<T> extends ConcreteType<T> implements CollectionType
{
    public final Kind kind;

    @SuppressWarnings("unchecked")
    protected ConcreteCollectionType(ComparisonType comparisonType, Kind kind)
    {
        super(comparisonType, (Class<T>) kind.type);
        this.kind = kind;
    }
    ...
    // Resolve the type conflict between getSerializer from CollectionType and ConcreteType by giving a more specific
    // type than the one in either.
    @Override
    ...
    public boolean isCompatibleWith(AbstractType previous)
    {
        ...
        ConcreteCollectionType<?> tprev = (ConcreteCollectionType<?>) previous;
    ...
    protected abstract boolean isCompatibleWithFrozen(ConcreteCollectionType<?> previous);

    /** A version of isValueCompatibleWith() to deal with non-multicell (frozen) collections */
    protected abstract boolean isValueCompatibleWithFrozen(ConcreteCollectionType<?> previous);
    ...
    public boolean equals(Object o, boolean ignoreFreezing)
    {
        ...
        if (!(o instanceof ConcreteCollectionType))
            return false;

        ConcreteCollectionType<?> other = (ConcreteCollectionType<?>)o;
