...
public class UnfilteredRowIteratorsMergeTest
{
    ...
    {
        testForInput("1[50] 2<[10] [10]<=3 3<[9] [9]<=3 3<[11] 4[50] [11]<5",
                     "3<[60] [60]<=5");
    }
    ...
    private List<Unfiltered> generateSource(Random r, Function<Integer, Integer> timeGenerator)
    {
        ...
        for (int i=0; i<ITEMS; ++i)
        {
            ...
            if (sz > 0 && r.nextBoolean())
            {
                ...
                content.add(new RangeTombstoneBoundMarker(boundFor(pos, true, pos > prev ? (span > 0 ? r.nextBoolean() : true) : false), dt));
                content.add(new RangeTombstoneBoundMarker(boundFor(pos + span, false, span < sz ? (span > 0 ? r.nextBoolean() : true) : false), dt));
            }
            else
            {
        ...
        try {
            ...
            for (Unfiltered unfiltered : list)
            {
                ...
                if (unfiltered.kind() == Kind.RANGE_TOMBSTONE_MARKER)
                {
                    ...
                    if (prev != null)
                    {
                        // The duplication below is technically an error, but isn't easy to fix. Permit for now.
                        // Assert.assertFalse("Duplicate marker " + str(prev) + " vs " + str(curr), comparator.compare(prev, curr) == 0);
