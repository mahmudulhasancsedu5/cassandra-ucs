...
public class RowCacheTest
{
    ...
    public void testRoundTrip() throws Exception
    {
        ...
        for (Unfiltered unfiltered : Util.once(cachedCf.unfilteredIterator(ColumnsSelection.selection(cachedCf.columns()), Slices.ALL, false)))
        {
    ...
    public void testRowCache() throws Exception
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            try (UnfilteredRowIterator ai = cp.unfilteredIterator(ColumnsSelection.selection(cp.columns()), Slices.ALL, false))
            {
                assert ai.hasNext();
                Row r = (Row)ai.next();
